    ...
    if (moduleAttachment) {
      return (
        <div className="space-y-1">
          <a href={moduleAttachment.url} download={moduleAttachment.name} className="text-sm text-blue-600 hover:underline">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 md:p-8">
            ...
            <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="flex flex-col gap-2">
                  ...
                  <input type="date" value={filterDate} onChange={(e) => setFilterDate(e.target.value)} className="w-full h-12 border border-gray-300 rounded-xl px-4 focus:outline-none focus:ring-2 focus:ring-blue-600" />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-semibold text-gray-700">Cari Nama Project</label>
                  <input type="text" value={projectSearch} onChange={(e) => setProjectSearch(e.target.value)} placeholder="Cari project..." className="w-full h-12 border border-gray-300 rounded-xl px-4 focus:outline-none focus:ring-2 focus:ring-blue-600" />
                </div>
              </div>
              {(filterDate || projectSearch) && (
                <button type="button" onClick={() => { setFilterDate(""); setProjectSearch(""); }} className="h-12 px-4 rounded-xl border border-gray-300 bg-white text-sm text-gray-700 hover:bg-gray-50">
                  Reset filter
                </button>
              )}
            </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 text-gray-500">Belum ada project yang diupload. Silakan tambah project di halaman Projects.</div>
          ) : filteredProjects.length === 0 ? (
            <div className="text-center py-16 text-gray-500">Tidak ada project untuk tanggal {filterDate}. Silakan hapus filter atau pilih tanggal lain.</div>
          ) : (
                    ...
                    return (
                      <tr key={project.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                        <td className="py-4 px-3">
                          {imageAttachment ? (
                            <button type="button" onClick={() => setSelectedProject(project)} className="group inline-flex items-center rounded-lg overflow-hidden">
                              <img src={imageAttachment.url} alt={imageAttachment.name} className="w-20 h-14 object-cover rounded-lg transition duration-200 group-hover:scale-105" />
                            </button>
                          ) : (
                        ...
                        <td className="py-4 px-3 text-gray-600">
                          {project.repoLink ? (
                            <a href={project.repoLink} target="_blank" rel="noreferrer" className="text-sm text-blue-600 hover:underline">Buka Link</a>
                          ) : (
                            <span className="text-sm text-gray-400">Tidak ada</span>
                          )}
                        </td>
                        ...
                        <td className="py-4 px-3">
                          <span className={`text-xs px-2 py-1 rounded ${project.decision === "approved" ? "bg-green-100 text-green-700" : project.decision === "rejected" ? "bg-red-100 text-red-700" : "bg-yellow-50 text-yellow-700"}`}>
        ...
        {selectedProject && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true" onClick={() => setSelectedProject(null)}>
            <div className="relative max-w-5xl w-full rounded-3xl overflow-hidden bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <button type="button" onClick={() => setSelectedProject(null)} className="absolute top-4 right-4 z-10 rounded-full bg-black/60 px-3 py-1 text-white text-xl">×</button>
                    ...
                    <>
                      <div className="space-y-4 overflow-y-auto max-h-[75vh]">
                        <div className="space-y-3">
                          ...
                          <input type="search" value={attachmentSearch} onChange={(e) => { setAttachmentSearch(e.target.value); setSelectedAttachmentIndex(0); }} placeholder="Cari PDF, gambar, atau link..." className="w-full h-12 border border-gray-300 rounded-xl px-4 focus:outline-none focus:ring-2 focus:ring-blue-600" />
                        </div>
                              ...
                              <div key={groupKey} className="space-y-2">
                                ...
                                <div className="space-y-2">
                                  {items.map((item) => {
                                    ...
                                    return (
                                      <div key={`attach-${groupKey}-${globalIndex}`} className={`w-full rounded-xl border p-3 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-2 justify-between min-w-0 ${selectedAttachmentIndex === globalIndex ? "border-blue-600 bg-blue-50" : "border-gray-200 bg-white hover:bg-gray-50"}`}>
                                        <button type="button" onClick={() => setSelectedAttachmentIndex(globalIndex)} className="flex items-center gap-3 text-left flex-1 min-w-0">
                                          {item.type === "image" ? (
                                        ...
                                        <div className="flex-none flex gap-2">
                                          <button type="button" onClick={() => handleDeleteAttachment(item)} className="text-red-600 hover:text-red-800 text-sm px-2 py-1 rounded-xl border border-red-200 bg-red-50" title="Hapus lampiran">Hapus</button>
                                        </div>
                                      </div>
                                    );
                                  })}
                        ...
                        {!filteredAttachments.length && <div className="text-sm text-gray-500">Tidak ada lampiran yang cocok dengan pencarian.</div>}
                      </div>
                        ...
                        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between md:flex-wrap">
                          ...
                          <div className="flex flex-col items-start gap-3 md:items-end">
                            ...
                            <div className="flex flex-wrap gap-2">
                              <button type="button" onClick={handleFinish} className="rounded-xl bg-[#001d55] px-4 py-2 text-sm font-semibold text-white hover:bg-[#00327a]">Finish</button>
                              {selectedProject.finished && <span className="text-xs px-2 py-1 rounded bg-green-100 text-green-700">Selesai</span>}
                            </div>
                          </div>
                        </div>
                          ...
                          return (
                            <div className="flex flex-col gap-4">
                              {attachment.type === "image" ? (
                                <img src={attachmentUrl} alt={attachment.name || selectedProject.name} className="w-full h-[60vh] object-contain rounded-3xl bg-black" />
                              ...
                              <div className="flex flex-wrap items-center gap-3">
                                ...
                                <button type="button" onClick={() => setAttachmentStatus(attachment, "approved")} className="rounded-xl bg-green-600 px-4 py-2 text-sm font-semibold text-white hover:bg-green-700">Approve Lampiran</button>
                                <button type="button" onClick={() => setAttachmentStatus(attachment, "rejected")} className="rounded-xl bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700">Reject Lampiran</button>
                                {attachment.type === "link" ? (
                                  <a href={attachmentUrl} target="_blank" rel="noreferrer" className="rounded-xl border border-blue-600 px-4 py-2 text-sm font-semibold text-blue-700 hover:bg-blue-50">Buka Link</a>
                                ) : attachment.type === "image" ? null : (
                                  <a href={attachmentUrl} download={attachment.name || "modul.pdf"} className="rounded-xl border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Unduh {attachment.name || attachment.label}</a>
                                )}
                              </div>
                            </div>
                          );
